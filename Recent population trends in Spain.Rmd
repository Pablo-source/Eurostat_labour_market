...
## 1. Load Spanish population data
...
excel_sheets("./data/INE total and foreign population figures Spain.xlsx")
```
...
foreign_pop_first_year <- INE_calc_fields  %>% select (Year,total_population) %>% filter(Year == min(INE_calc_fields$Year))
Min_total_population

foreign_pop_last_year <- INE_calc_fields  %>% select (Year,total_population) %>% filter(Year == max(INE_calc_fields$Year))
...
- In terms of foreign population, 
